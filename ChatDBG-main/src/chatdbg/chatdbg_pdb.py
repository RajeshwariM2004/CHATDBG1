...
class ChatDBG(ChatDBGSuper):
    ...
    def _initial_prompt_suspicious_frames(self, limit: int = 5) -> str:
        """
        Basic fault localization:
        Heuristically rank and list the most suspicious user-code frames.

        Strategy:
        - Walk the stack from most recent to oldest.
        - Keep only frames that look like user code (not stdlib / site-packages).
        - Show up to `limit` frames with filename, line number, function and source line.
        """
        if not hasattr(self, "stack") or not self.stack:
            return ""

        lines = []
        for frame, lineno in self.stack[::-1]:  # most recent first
            ...
            try:
                ...
                src = ""
            rank = len(lines) + 1
            snippet = f"{rank}. {filename}:{lineno} in {func_name}"
            if src:
                snippet += f"\n       {src}"
        ...
        header = (
            "Suspicious user-code locations (most recent frames first). "
            "These are good starting points for root-cause analysis.\n"
        )
        return header + "\n".join(lines)
    ...
    def _build_prompt(self, arg, conversing):
        if not conversing:
            return build_initial_prompt(
                ...
                extra=self._initial_prompt_suspicious_frames(),
                user_text=arg,
            )
        else:
