...
class Assistant:
    def __init__(
        self,
        instructions,
        # Default to OpenAI GPT-4o; can be overridden via config / CLI
    ...
    def query(self, prompt: str, user_text):
        ...
        try:
            ...
            # Check if it's an API error (could be from any provider)
            error_str = str(e).lower()
            if "api" in error_str or "key" in error_str or "auth" in error_str:
                self._warn_about_exception(e, f"API Error.  Please check your API key and try again.")
            else:
    ...
    def _check_model(self):
        ...
        if missing_keys != []:
            ...
            # Generic message that works for Groq and any other provider
            raise AssistantError(
                    textwrap.dedent(
                        f"""\
                    You need to set the following environment variables
                to use the {self._model} model (provider: {provider}): {', '.join(missing_keys)}."""
                    )
                )
        ...
        try:
            if not litellm.supports_function_calling(self._model):
                raise AssistantError(
                    ...
                    You must use a model that does, eg. groq/llama-3.3-70b-versatile."""
                    )
                )
        except:
            raise AssistantError(
    ...
    def _stream_completion(self):
        ...
        # Prefer OpenAI first, but fall back to other providers if present.
        api_key = (
